...
def full_runge_error(h, p=2):        #Оценка полной пограшности метода Ньютона_Эйлера
    ...
    if (flag2 == 1):        #Предпочтительнее брать h/2
        ...
        h, Rn = full_runge_error(h)
        return h, Rn

'''
Автоматический выбор шага интегрирования для метода Ньютона-Эйлера
'''
def run_aut_NE(rtol, atol=1e-12, h=find_step(tol),  x=x_0, y1=y1_0, y2=y2_0, y3=y3_0, y4=y4_0, p=2):
...
def rung_auto_op(rtol, atol=1e-12, h=find_step(tol),  x=x_0, y1=y1_0, y2=y2_0, y3=y3_0, y4=y4_0, p=2):
    X_runge = [x]       #Массивы для построения графиков
    Y1_runge = [y1]
    Y2_runge = [y2]
    Y3_runge = [y3]
...
h_tol, Rn = full_runge_error(find_step(tol)) #вычисляем шаг через оценку полной погрешности для оценки точности метода
...
X_runge, Y1_runge, Y2_runge, Y3_runge, Y4_runge, H, R = run_aut_NE(rtol = 1e-8)
stop_time = time.time()
